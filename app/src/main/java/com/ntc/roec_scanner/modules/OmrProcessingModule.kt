...
import com.ntc.roec_scanner.utils.rotateBitmapIfNeeded
...
fun analyzeImageFile(
    ...
    context.contentResolver.openInputStream(imageUri)?.use { input ->
        ...
        val warped =
            if (corners != null) warpSheetFromPoints(finalMat, corners.toTypedArray()) else null
        ...
        val debugBitmap = processAnswerSheetWithEnsemble(
            context,
            warped,
            qrData,
            detectedAnswers,
    ...
    for ((columnIndex, col) in columns.withIndex()) {
        ...
        for (q in 0 until questions) {
            ...
            val detectedValue = when {
                ...
                second.second > params.invalidThreshold && second.second > (minOf(
                    best.second,
                    1.0
                ) * params.dominanceRatio) -> -8
            ...
            Log.d(
                "OMR_METRICS",
                "[$sweepName] Test $realTestNumber Q${q + 1} -> Det: $detStr | 1st: $bestChar (${best.second.fmt()}), 2nd: $secondChar (${second.second.fmt()})"
            )
...
fun processAnswerSheetWithEnsemble(
    ...
    val parameterSweep = listOf(
        ...
        OMRParams(
            ...
            staticBlockSize,
            hardMinMark = 0.01,
            invalidThreshold = 0.02,
            dominanceRatio = 0.075
        ),
        OMRParams(
            staticCValue,
            staticBlockSize,
            hardMinMark = 0.01,
            invalidThreshold = 0.03,
            dominanceRatio = 0.1
        ),
        OMRParams(
            staticCValue,
            staticBlockSize,
            hardMinMark = 0.01,
            invalidThreshold = 0.04,
            dominanceRatio = 0.125
        ),
        OMRParams(
            staticCValue,
            staticBlockSize,
    ...
    for (i in 0 until numQuestions) {
        ...
        val validConsensus =
            if (mostCommonValid != null) validVotes.count { it == mostCommonValid } else 0

        // Grab the most common error code to log if the ensemble fails this question
        val mostCommonInvalid =
            votes.filter { it <= -2 }.groupingBy { it }.eachCount().maxByOrNull { it.value }?.key
                ?: -2
        ...
        val strayChar =
            if (strayVote != null && strayVote > 0) ('A' + strayVote - 1).toString() else strayVote.toString()

        Log.d(
            "OMR_ENSEMBLE",
            "Test ${allScans.first()[i].testNumber} Q${allScans.first()[i].questionNumber} | Votes: $voteChars | Stray: $strayChar -> FINAL: $finalStr"
        )
        ...
        for (q in 0 until questions) {
            ...
            if (detectedValue in 1..choices) {
                ...
                if (isCorrect) {
                    if (showCorrect) {
                        val circleColor =
                            if (consensusScore == 5) Scalar(0.0, 255.0, 0.0, 255.0) else Scalar(
                                255.0,
                                255.0,
                                0.0,
                                255.0
                    ...
                    if (showIncorrect) {
                        val xColor =
                            if (consensusScore == 5) Scalar(255.0, 0.0, 0.0, 255.0) else Scalar(
                                255.0,
                                165.0,
                                0.0,
                                255.0
                ...
                if (showDouble) {
                    Imgproc.line(
                        mat,
                        Point(xStart.toDouble(), centerY.toDouble()),
                        Point(xEnd.toDouble(), centerY.toDouble()),
...
fun reprocessWithNewCorners(
    ...
    val detectedAnswers = mutableListOf<DetectedAnswer>()
    val cleanBitmap = processAnswerSheetWithEnsemble(
        context,
        warped,
        qrData,
        detectedAnswers,
